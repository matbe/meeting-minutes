...
- Uses `token=` parameter (not deprecated `use_auth_token=`)
- Model.from_pretrained() + Inference API unchanged
- pyannote.core.Segment API unchanged
"""
import logging
import numpy as np
...
# Maximum embeddings to keep per speaker profile (prevents memory bloat)
...
class SpeakerProfile:
    ...
    def add_embedding(self, embedding: np.ndarray, duration: float = 0.0):
        ...
        if len(self.embeddings) > MAX_EMBEDDINGS_PER_SPEAKER:
            self.embeddings = self.embeddings[-MAX_EMBEDDINGS_PER_SPEAKER:]
...
class SessionSpeakerStore:
    ...
    def add_speaker(
        ...
        embedding: np.ndarray, duration: float = 0.0
    ):
    ...
    def _load_session(self, session_id: str):
        ...
        try:
            ...
            # Find all speaker IDs from saved centroids
            speaker_ids = set()
            ...
            for speaker_id in speaker_ids:
                ...
                duration = float(data.get(f"{speaker_id}_duration", [0.0])[0])
...
class SpeakerTracker:
    ...
    def _load_embedding_model(self):
        ...
        try:
            ...
            if device.type != "cpu":
                self.embedding_model.model = self.embedding_model.model.to(device)
    ...
    def find_matching_speaker(
        self, session_id: str, embedding: np.ndarray
    ...
    def assign_speakers(
        ...
        Args:
            ...
            num_speakers: Optional hint for max number of speakers
        ...
        logger.info(
            f"Assigning speakers for session {session_id}, "
            f"{len(diarization_turns)} turns"
        )
        ...
        for local_label, turns in local_speaker_turns.items():
            ...
            if match_id and similarity >= SPEAKER_MATCH_THRESHOLD:
                ...
                logger.debug(
                    f"Matched {local_label} -> {match_id} (similarity: {similarity:.3f})"
                )

                # Only persist embedding if duration is sufficient
                if duration >= MIN_DURATION_TO_PERSIST:
                    self.store.add_speaker(session_id, match_id, embedding, duration)
            else:
                # Check if we've hit the speaker limit
                profiles = self.store.get_session(session_id)
                if num_speakers and len(profiles) >= num_speakers:
                    ...
                    if match_id:
                        label_mapping[local_label] = match_id
                        logger.debug(
                            f"At speaker limit, best match: {local_label} -> {match_id} "
                    ...
                    logger.info(
                        f"New speaker: {local_label} -> {new_id} "
